package com.fancheerinteractive.emc.service.servlet;
...
public class CSRFHandlerInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		// LoginController -- the only service which doesn't get checked for
		// sign on.
		// if (handler.getClass().getName().equals(LOGIN_CONTROLLER_CLASS_NAME))
		// {
		// return true;
		// }
		logger.debug("^^^^^^^^^^^^^^^^^^ preHandling request URI "
		...
		if ("POST".equalsIgnoreCase(request.getMethod())
			...
			if (sessionToken.equals(requestToken)) {
				...
				logger.error("^^^^^^^^^^^^^^^^^^^^^^^^ Possible CSRF attack! "
						+ request.getRequestURI());
	...
	public void postHandle(HttpServletRequest request,
		...
		logger.debug("^^^^^^^^^^^^^^^^^^ postHandling request URI "
				+ request.getRequestURI());
